...
for iBeam = 2   % 1 = dics | 2 = lcmv
    ...
    for isubj=1:length(goodsub);
        ...
        for icond=1:6;
            ...
            config.data.timewin     = [-.75 1.0];
            config.bsln.timewin     = [-.75 0.0];
            config.actv.timewin     = [ 0 1.0];
            
            %sam and lcmv options
            config.cov.timewin      = [0 0.2];
            config.cov.bpfreq       = [1 40];            
         
            %dics options
            config.csd.timewin      = [-.75 1.0];
            ...
            %config.src.sub = ([num2str(isub), '_', beam_method_arr{iBeam}]);
            ...
            %run the analysis
            [src_diff, src_peak, data_virt, config] = cub_ft_beam_baselinecond_kp2(config);
            src_diff=rmfield(src_diff,'cfg');
            %save the output
            str_append = ['_' config.method.beam '_' config.method.dfit '_' config.method.forw_str '_' config.method.diff_str];
            file_savename = fullfile(config.subject_path, strrep(config.mrifile_name, 'MRI', [str_append '_out.mat']));
